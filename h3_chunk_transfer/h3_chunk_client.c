...
typedef struct h3_chunk_worker_slot_s {
    pthread_t thread;
    h3_chunk_client_worker_ctx *worker;
    int active;
} h3_chunk_worker_slot;

static void
h3_chunk_client_usage(const char *prog)
{
    fprintf(stderr,
...
static void
h3_chunk_client_init_config(h3_chunk_client_config *config)
{
    memset(config, 0, sizeof(*config));
    snprintf(config->server_addr, sizeof(config->server_addr), "%s", H3_CHUNK_DEFAULT_ADDR);
    snprintf(config->server_host, sizeof(config->server_host), "%s", H3_CHUNK_DEFAULT_HOST);
    config->server_port = H3_CHUNK_DEFAULT_PORT;
    config->chunk_size = H3_CHUNK_DEFAULT_CHUNK_SIZE;
    config->concurrency = H3_CHUNK_DEFAULT_CONCURRENCY;
    config->max_retries = H3_CHUNK_DEFAULT_RETRIES;
    config->timeout_sec = H3_CHUNK_DEFAULT_TIMEOUT_SEC;
    config->log_level = CHUNK_LOG_INFO;
}

static int
h3_chunk_client_parse_args(h3_chunk_client_config *config, int argc, char **argv)
{
    int opt;
    uint32_t parsed;
    ...
    while ((opt = getopt(argc, argv, "a:p:h:i:k:j:r:t:l:")) != -1) {
        ...
        case 'i':
            ...
            break;
        case 'k':
            if (h3_chunk_parse_u32_arg(optarg, &config->chunk_size) != 0
                || config->chunk_size == 0)
            {
                return -1;
            }
            break;
        case 'j':
            if (h3_chunk_parse_u32_arg(optarg, &config->concurrency) != 0
            ...
            break;
        case 'r':
            if (h3_chunk_parse_u32_arg(optarg, &config->max_retries) != 0) {
                return -1;
            }
            break;
        case 't':
...
static int
h3_chunk_client_launch_worker(h3_chunk_worker_slot *slot, const h3_chunk_client_config *config,
    ...
    pthread_cond_t *cond)
{
    ...
    worker = (h3_chunk_client_worker_ctx *)calloc(1, sizeof(*worker));
    if (worker == NULL) {
        return -1;
    }
    ...
    rc = pthread_create(&slot->thread, NULL, h3_chunk_client_worker_thread_main, worker);
    if (rc != 0) {
        free(worker);
        return -1;
    }
...
static int
h3_chunk_client_has_finished_worker(h3_chunk_worker_slot *slots, uint32_t slot_count)
{
    uint32_t i;
...
static void
h3_chunk_client_join_finished(h3_chunk_worker_slot *slots, uint32_t slot_count,
    uint32_t chunk_count, const h3_chunk_client_config *config, uint32_t *active_workers,
    uint32_t *completed_chunks, uint32_t *queue, size_t *queue_tail, int *exhausted)
{
    uint32_t i;
    ...
    for (i = 0; i < slot_count; ++i) {
        ...
        if (worker->result.success) {
            (*completed_chunks)++;
            chunk_log_print(config->log_level, CHUNK_LOG_INFO, "h3_chunk_client",
            ...
            chunk_log_print(config->log_level, CHUNK_LOG_WARN, "h3_chunk_client",
...
main(int argc, char **argv)
{
    ...
    h3_chunk_worker_slot *slots = NULL;
    uint32_t *queue = NULL;
    size_t queue_cap;
    size_t queue_head = 0;
    size_t queue_tail = 0;
    ...
    h3_chunk_client_init_config(&config);
    if (h3_chunk_client_parse_args(&config, argc, argv) != 0) {
        h3_chunk_client_usage(argv[0]);
        return 1;
    }
    ...
    slots = (h3_chunk_worker_slot *)calloc(config.concurrency, sizeof(*slots));
    ...
    chunk_log_print(config.log_level, CHUNK_LOG_INFO, "h3_chunk_client",
        "start HTTP/3 transfer file=%s file_id=%" PRIu64 " size=%" PRIu64
        " chunk_count=%u chunk_size=%u concurrency=%u retries=%u",
    ...
    while (completed_chunks < chunk_count && !retries_exhausted) {
        for (i = 0; i < config.concurrency && queue_head < queue_tail; ++i) {
            ...
            if (h3_chunk_client_launch_worker(&slots[i], &config, file_id, file_size,
        ...
        while (!h3_chunk_client_has_finished_worker(slots, config.concurrency)) {
            pthread_cond_wait(&scheduler_cond, &scheduler_mutex);
        }
        pthread_mutex_unlock(&scheduler_mutex);

        h3_chunk_client_join_finished(slots, config.concurrency, chunk_count, &config,
...
join_active:
    ...
    if (overall_success) {
        chunk_log_print(config.log_level, CHUNK_LOG_INFO, "h3_chunk_client",
            "HTTP/3 transfer completed successfully, file_id=%" PRIu64, file_id);
    }
